...
```shell
$ cargo run
Err(Error("Target satisfies the `Not` rule", line: 2, column: 18))
```
...
```shell
$ cargo run
Err(Error("10 is not greater than 18", line: 3, column: 17))
```
...
```shell
$ cargo run
Err(Error("Target satisfies the `Not` rule", line: 5, column: 5))
```
...
```shell
$ cargo run
Err(Error("Target satisfies the `Not` rule", line: 4, column: 30))
```
